...
<div x-data="autocomplete({ event: '{{ $event }}' })" wire:model.live="autocompleteItems" x-modelable="items" class="relative w-full" @click.outside.capture="isTyped=false;">
    <div @keyup.escape.window="isTyped = false" @click.outside="isTyped = false">
        <div class="flex items-center">
            ...
            <x-icons icon="circle-x" class="-ml-8 text-red-600" @click="clear" x-show="isTyped" />
        </div>
        <div x-show="isTyped" x-cloak class="absolute z-10 w-full" wire:key="{{ time() }}">
            <ul class="z-50 max-h-72 w-full cursor-pointer overflow-y-auto rounded-xs border border-t-0 border-gray-300 bg-white">
                <template x-for="(item, index) in items">
                    <li class="px-2 py-1 text-sm" :x-ref="index" x-text="item.name" :class="{ 'bg-gray-300': focus === index }" @click="selectItem" @mouseenter="setFocus(index)"></li>
                </template>
            </ul>
        </div>

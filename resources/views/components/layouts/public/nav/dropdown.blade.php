...
<div x-cloak>
    <div
        ...
        x-data="{
            ...
            scheduleClose() {
                this.clearClose()
                this.closeTimer = setTimeout(() => (this.menuOpen = false), 250) // 250ms delay
            },
            clearClose() {
                this.menuOpen = true
                if (this.closeTimer) {
    ...
    <div class="block lg:hidden" x-data="{
        expanded: false,
        activeClass: '{{ request()->route()->getName() == $route }}',
        init() {
            this.expanded = this.activeClass
        },
    }" {{ $attributes }}>
        <button @click="expanded = ! expanded">
            <span class="-mx-3 flex items-center rounded-lg px-3 py-2 text-base leading-7 font-medium text-zinc-500 hover:text-primary sm:mx-0 sm:text-sm dark:text-zinc-200" :class="activeClass && '!font-semibold !text-primary'">
                {{ __($menuTitle) }}
                <x-icons icon="chevron-right" class="h-2 w-2" ::class="expanded  && '!font-semibold !text-primary origin-center rotate-90 text-primary-600'" />
            </span>
        </button>

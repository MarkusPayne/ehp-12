...
<x-layouts.public>
    ...
    <x-page-section>
        <x-page-section-content title="News">
            <div
                class="mx-auto mt-10 grid max-w-2xl grid-cols-1 gap-x-8 gap-y-10 divide-y divide-gray-200 lg:mx-0 lg:max-w-none lg:grid-cols-3">
                @foreach (PublicContentService::getNews() as $article)
                    <article class="flex max-w-xl flex-col items-start justify-between pb-10">
                        ...
                        <div class="group relative grow">
                            ...
                            <h6 class="mt-3 pb-3 text-lg/6 font-semibold text-gray-700 group-hover:text-gray-600">
                                {{ $article->sub_title }}
                            </h6>
                            {!! $article->blurb !!}
                        </div>
                        <div class="relative mt-8 flex items-center gap-x-4 justify-self-end">
                            <a href="@if($article->news_type_id == 3){{ Storage::url($article->link) }}@else {{ $article->link }}@endif"
                               class="btn-news" target="_blank">
                                @if ($article->news_type_id != 1)
                                    Read More
                                @else
                                    Watch Video
                                @endif
